...
Example usage:
    python object_detection/dataset_tools/create_pascal_tf_record.py \
        ...
        --proposal_data_path=data/proposal_data
"""
...
flags.DEFINE_boolean('ignore_difficult_instances', False, 'Whether to ignore '
                     'difficult instances')
flags.DEFINE_string('proposal_data_path', 'raw_data/proposal_data_path',
                    'Path to label map proto')
flags.DEFINE_integer('num_shards', 20, 'Number of output parts.')
...
def dict_to_tf_example(data,
  ...
  filename = os.path.join(proposal_data_path, '{}.npy'.format(image_id))
...
def main(_):
  ...
  for i in range(FLAGS.num_shards):
    filename = FLAGS.output_path + '-%05d-of-%05d' % (i, FLAGS.num_shards)
